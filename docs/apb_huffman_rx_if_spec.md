...
| `0x0C` | `RX_CONTROL` | W | bit0 soft reset, bit1 clear done flags, bit2 clear error |
...
| `21:18` | legacy ciphertext staging valid bits |
| `22` | legacy ciphertext staging complete |
| `23` | legacy ciphertext pending valid |
| `24` | legacy ciphertext pending can accept |
...
`RX_CONTROL` write bits:

| Bit | Meaning |
|---:|---|
| `0` | soft reset local RX IF FIFO/sticky/staging state |
| `1` | clear block/frame done sticky flags |
| `2` | clear error sticky |
| `31:3` | reserved; writing `1` causes `PSLVERR` |

`CTXT_STATUS` exposes the legacy ciphertext staging path:

| Bit | Meaning |
|---:|---|
| `3:0` | staging word valid bits for `CTXT_W0..W3` |
| `4` | all four staging words valid |
| `5` | pending 128-bit ciphertext word valid |
| `6` | pending slot can accept a new staged word |
| `7` | downstream ciphertext stream ready |
