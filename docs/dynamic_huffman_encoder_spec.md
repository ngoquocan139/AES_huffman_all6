...
-> apb_huffman_tx_if
-> huffman_aes_tx_top
   -> whole-file byte feed for count/emit
...
![dynamic_huffman_encoder detailed block diagram](./assets/dynamic_huffman_compression_block_diagram.svg)

Figure. Datasheet-style block diagram of the whole-file dynamic Huffman
compression path, aligned with the thesis subsections 3.4.2.1, 3.4.2.2, and
3.4.2.3. The green stage corresponds to input collection, frequency counting,
and code-length derivation. The amber stage corresponds to canonical code
generation and canonical codebook construction. The blue stage corresponds to
...
```mermaid
flowchart LR
  IN[/"whole-file payload bytes"/] --> COL["collect current <=32-byte block"]
...
| `byte_in`, `byte_valid`, `byte_ready` | Byte-stream handshake for the current whole-file payload block |
