...
module test_bench;
    ...
    integer start_pulse_base;
    integer cipher_pulse_base;
    integer tx_done_pulse_base;
    ...
    // --------------------------------------------------------------------
    // APB write
    // Sample PREADY/PSLVERR at negedge during ACCESS.
    // --------------------------------------------------------------------
    task apb_write;
        input  [31:0] addr;
        input  [31:0] data;
    ...
    initial begin
        ...
        // ================================================================
        // TEST 1: VALID BLOCK 10 BYTES
        // "ABCDEFGHIJ"
        // ================================================================
        $display("\n===== TEST 1: VALID BLOCK 10 BYTES =====");
        start_pulse_base   = start_pulse_count;
        cipher_pulse_base  = cipher_pulse_count;
        tx_done_pulse_base = tx_done_pulse_count;

        apb_write(ADDR_BLOCK_SIZE, 32'h0000_000A, apb_err, apb_waits);
        check_bit(apb_err, 1'b0, "t1_block_size_write_should_succeed");

        apb_write(ADDR_WORD_IN, 32'h44434241, apb_err, apb_waits); // ABCD
        check_bit(apb_err, 1'b0, "t1_word0_should_succeed");

        apb_write(ADDR_WORD_IN, 32'h48474645, apb_err, apb_waits); // EFGH
        check_bit(apb_err, 1'b0, "t1_word1_should_succeed");

        apb_write(ADDR_WORD_IN, 32'h00004A49, apb_err, apb_waits); // IJ
        check_bit(apb_err, 1'b0, "t1_word2_should_succeed");

        apb_read(ADDR_STATUS, apb_rdata, apb_err, apb_waits);
        check_bit(apb_rdata[7], 1'b1, "t1_can_start_should_be_1");

        apb_write(ADDR_START_BLOCK, 32'h0000_0001, apb_err, apb_waits);
        check_bit(apb_err, 1'b0, "t1_start_block_should_succeed");

        wait_done_or_error(done_flag, err_flag, 5000);
        check_int(done_flag,  1, "t1_done_flag_should_be_1");
        check_int(err_flag,   0, "t1_error_flag_should_be_0");

        apb_read(ADDR_STATUS, apb_rdata, apb_err, apb_waits);
        check_bit(apb_rdata[4], 1'b1, "t1_done_sticky_should_be_1");
        check_bit(apb_rdata[5], 1'b0, "t1_error_sticky_should_be_0");

        check_int(start_pulse_count - start_pulse_base, 1, "t1_start_block_pulse_count_delta_should_be_1");
        check_bit((cipher_pulse_count - cipher_pulse_base) > 0, 1'b1, "t1_cipher_en_should_pulse_at_least_once");
        check_bit((tx_done_pulse_count - tx_done_pulse_base) > 0, 1'b1, "t1_tx_done_should_pulse");

        apb_write(ADDR_CONTROL, 32'h0000_0002, apb_err, apb_waits); // clear_done
        check_bit(apb_err, 1'b0, "t1_clear_done_should_succeed");

        // ================================================================
        // TEST 2: INVALID BLOCK_SIZE = 0
        // ================================================================
        ...
        apb_write(ADDR_CONTROL, 32'h0000_0004, apb_err, apb_waits); // clear_error
        check_bit(apb_err, 1'b0, "t2_clear_error_should_succeed");

        // ================================================================
        // TEST 3: VALID BLOCK 32 BYTES
        // 8 words
        // ================================================================
        $display("\n===== TEST 3: VALID BLOCK 32 BYTES =====");
        start_pulse_base   = start_pulse_count;
        ...
        apb_write(ADDR_BLOCK_SIZE, 32'h0000_0020, apb_err, apb_waits);
        check_bit(apb_err, 1'b0, "t3_block_size_32_should_succeed");

        apb_write(ADDR_WORD_IN, pack4("A","B","C","D"), apb_err, apb_waits); check_bit(apb_err, 1'b0, "t3_word0_ok");
        apb_write(ADDR_WORD_IN, pack4("E","F","G","H"), apb_err, apb_waits); check_bit(apb_err, 1'b0, "t3_word1_ok");
        apb_write(ADDR_WORD_IN, pack4("I","J","K","L"), apb_err, apb_waits); check_bit(apb_err, 1'b0, "t3_word2_ok");
        apb_write(ADDR_WORD_IN, pack4("M","N","O","P"), apb_err, apb_waits); check_bit(apb_err, 1'b0, "t3_word3_ok");
        apb_write(ADDR_WORD_IN, pack4("Q","R","S","T"), apb_err, apb_waits); check_bit(apb_err, 1'b0, "t3_word4_ok");
        apb_write(ADDR_WORD_IN, pack4("U","V","W","X"), apb_err, apb_waits); check_bit(apb_err, 1'b0, "t3_word5_ok");
        apb_write(ADDR_WORD_IN, pack4("Y","Z","1","2"), apb_err, apb_waits); check_bit(apb_err, 1'b0, "t3_word6_ok");
        apb_write(ADDR_WORD_IN, pack4("3","4","5","6"), apb_err, apb_waits); check_bit(apb_err, 1'b0, "t3_word7_ok");

        apb_write(ADDR_START_BLOCK, 32'h0000_0001, apb_err, apb_waits);
        check_bit(apb_err, 1'b0, "t3_start_block_should_succeed");

        wait_done_or_error(done_flag, err_flag, 10000);
        check_int(done_flag,  1, "t3_done_flag_should_be_1");
        check_int(err_flag,   0, "t3_error_flag_should_be_0");

        check_int(start_pulse_count - start_pulse_base, 1, "t3_start_block_pulse_count_delta_should_be_1");
        check_bit((cipher_pulse_count - cipher_pulse_base) > 0, 1'b1, "t3_cipher_en_should_pulse_at_least_once");
        check_bit((tx_done_pulse_count - tx_done_pulse_base) > 0, 1'b1, "t3_tx_done_should_pulse");

        apb_write(ADDR_CONTROL, 32'h0000_0002, apb_err, apb_waits); // clear_done
        check_bit(apb_err, 1'b0, "t3_clear_done_should_succeed");

        // ================================================================
        // TEST 4: INVALID READ ADDRESS
        // ================================================================
        $display("\n===== TEST 4: INVALID READ ADDRESS =====");
        apb_read(ADDR_INVALID, apb_rdata, apb_err, apb_waits);
        check_bit(apb_err, 1'b1, "t4_invalid_read_should_error");

        apb_write(ADDR_CONTROL, 32'h0000_0004, apb_err, apb_waits); // clear_error
        check_bit(apb_err, 1'b0, "t4_clear_error_should_succeed");
        ...
        check_bit(apb_err, 1'b0, "t5_soft_reset_should_succeed");

        apb_read(ADDR_STATUS, apb_rdata, apb_err, apb_waits);
        check_bit(apb_rdata[0], 1'b0, "t5_cfg_valid_should_be_0");
        check_bit(apb_rdata[2], 1'b0, "t5_block_active_should_be_0");
        check_bit(apb_rdata[4], 1'b0, "t5_done_sticky_should_be_0");
        check_bit(apb_rdata[5], 1'b0, "t5_error_sticky_should_be_0");

        $display("\n========================================");
        $display("APB_HUFFMAN_AES_FULL_TOP TEST SUMMARY: PASS=%0d FAIL=%0d", pass_count, fail_count);
        $display("========================================");
