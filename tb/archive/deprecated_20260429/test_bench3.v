...
module test_bench;
    ...
    integer                     total_aes_input_bits;
    integer                     saved_bits;
    integer                     saved_bytes_ceil;
    real                        total_compress_pct;

    integer                     blk_input_bits;
    integer                     blk_output_bits;
    integer                     blk_output_bytes_ceil;
    ...
    // ------------------------------------------------------------
    // Capture accepted AES input blocks
    // ------------------------------------------------------------
    ...
    begin
        total_output_bits        = 0;
        total_aes_input_bits     = cap_count * 128;
        total_output_bytes_ceil  = 0;
        saved_bits               = 0;
        saved_bytes_ceil         = 0;
        total_compress_pct       = 0.0;

        for (i = 0; i < cap_count; i = i + 1)
            total_output_bits = total_output_bits + cap_valid_bits[i];

        total_input_bits        = text_len * 8;
        total_output_bytes_ceil = (total_output_bits + 7) / 8;
        saved_bits              = total_input_bits - total_output_bits;
        saved_bytes_ceil        = (saved_bits + 7) / 8;

        if (total_input_bits > 0)
            total_compress_pct = (100.0 * (total_input_bits - total_output_bits)) / total_input_bits;
        else
            total_compress_pct = 0.0;
        ...
        $display("output blocks       = %0d", cap_count);
        $display("output bits         = %0d", total_output_bits);
        $display("output bytes ceil   = %0d", total_output_bytes_ceil);
        $display("saved bits          = %0d", saved_bits);
        $display("saved bytes ceil    = %0d", saved_bytes_ceil);
        $display("compression percent = %0.2f%%", total_compress_pct);
        $display("aes input bits      = %0d", total_aes_input_bits);
    ...
    begin
        ...
        for (j = start_cap_idx; j < cap_count; j = j + 1)
            blk_output_bits = blk_output_bits + cap_valid_bits[j];

        blk_output_bytes_ceil = (blk_output_bits + 7) / 8;
        blk_saved_bits        = blk_input_bits - blk_output_bits;
        ...
        $display("  output bits total    = %0d", blk_output_bits);
        $display("  output bytes ceil    = %0d", blk_output_bytes_ceil);
        $display("  saved bits           = %0d", blk_saved_bits);
        $display("  compression percent  = %0.2f%%", blk_compress_pct);
