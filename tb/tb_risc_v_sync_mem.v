...
// Keep the top-level module name identical to tb_risc_v.v so the same
// Makefile/TB_NAME can be reused. Select this file from sim/tb.f when
// testing synchronous memory behavior.
module test_bench;
  ...
  initial begin
    ...
    if (fail_count == 0)
      $display("[PASS] rv32i_sync_mem_smoke_test");
    else
      $display("[FAIL] rv32i_sync_mem_smoke_test");

    $writememh("regfile_dump_sync.txt", dut_reg_file.registers);
    $finish;
  end
endmodule
