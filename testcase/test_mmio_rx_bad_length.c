...
#define RESULT_SIGNATURE    0x52584552u
...
int main(void) {
    ...
    DMA_SRC_ADDR = TX_DST_BASE_ADDR;
    DMA_DST_ADDR = RX_DST_BASE_ADDR;
    DMA_LEN_BYTES = 0x00000004u;
    ...
    if (status_before != 0x00000028u)
        error_mask |= (1u << 0);
    if ((status_after & 0x00000004u) == 0u)
        error_mask |= (1u << 1);
    if (bytes_done != 0u)
        error_mask |= (1u << 2);
    if ((debug_after & 0x00000ff0u) != 0x00000020u)
        error_mask |= (1u << 3);
    if (polls >= MAX_POLLS)
        error_mask |= (1u << 4);
